	<header class="main-page-header">
		<div class="container-fluid">
			<div class="row header-mobile d-flex d-lg-none">
				<div class="col-auto">
					<div class="menuFive menuFiveMob">
	                     <span></span>
	                     <span></span>
	                     <span></span>
	                </div>
				</div>
				<div class="men">
					<ul class="topmenu">
						<li><a href="">О компании</a></li>
						<li> <div class="li-div"> <a href="" class="submenu-link">Для клиента </a> <div class="redArrow"></div></div>
							<ul class="submenu">
								<li><a href="">Страница 111</a></li>
								<li><a href="">Страница</a></li>
								<li><a href="">Страница</a></li>
							</ul>
						</li>
						<li><a href="">Услуги</a></li>
						<li><div class="li-div"><a href="" class="submenu-link">Каталог оборудования</a> <div class="redArrow"></div></div>
							<ul class="submenu">
								<li><a href="">Страница</a></li>
								<li><a href="">Страница</a></li>
								<li><a href="">Страница</a></li>
							</ul>
						</li>
						<li><a href="">Новости</a></li>
						
						<li><a href="">Статьи </a></li>
						<li><a href="">Контакты</a></li>
					</ul>
					<div class="last-mobile-line">
						<div class="col-12">
							<p><a href="mailto:manager@uralteh.ru">manager@uralteh.ru</a></p>
						</div>
						<div class="col-6">
							<p><a href="tel:(3412)600-999">(3412) 600-999</a> <br> <span>Ижевск</span></p>
						</div>
						<div class="col-6">
							<p><a href="tel:8(800)100-53-83">8 (800) 100-53-83</a> <br> <span>Регионы</span></p>
						</div>
						<div class="col-12 justify-content-between d-flex">
							<div>
								<a data-fancybox data-animation-duration="700" data-src="#orderCall" href="javascript:;">
									<button class="redBtn">Заказать звонок</button>
								</a>
							</div>
							<div>
								<a class="placemark" data-fancybox data-animation-duration="700" data-src="#showMap" href="javascript:;">
									<img src="img/icons/placemarkBlack.svg" alt="Карты">
								</a>
							</div>
						</div>
					</div>
				</div>
				<div class="col-auto">
					<a href="index.php" class="mobile-home-link">
						<img src="img/logo-mobile.png">
					</a>
				</div>
				<div class="col-auto">
					<form id="search2" class="search">
						<input type="search" placeholder="Поиск">
					</form>
				</div>
			</div>
			<div class="row">
				<div class="col-lg-3 d-none d-lg-block">
					<div class="additional-header">
						<div class="additional-header__logo">
							<img src="img/logo.png" alt="Логотип">
						</div>
						<div class="additional-header__products">
							<ul>
								<li><a href="">Краны грузоподъемные</a></li>
								<li><a href="">Тали</a></li>
								<li><a href="">Краны грузоподъемные</a></li>
								<li><a href="">Тали</a></li>
								<li><a href="">Краны грузоподъемные</a></li>
								<li><a href="">Тали</a></li>
								<li><a href="">Краны грузоподъемные</a></li>
								<li><a href="">Тали</a></li>
								<li><a href="">Краны грузоподъемные</a></li>
								<li><a href="">Тали</a></li>
								<li><a href="">Краны грузоподъемные</a></li>
								<li><a href="">Тали</a></li>
							</ul>
						</div>
						<div class="additional-header_links">
							<p><a href="">скачать каталог</a></p>
							<p><a href="">скачать прайс</a></p>
						</div>
						<div class="additional-header_social">
							<a href="">
								<img src="img/icons/skype.svg" alt="skype">
							</a>
							<a href="">
								<img src="img/icons/whatsapp.svg" alt="whatsapp">
							</a>
							<a href="">
								<img src="img/icons/viber.svg" alt="viber">
							</a>
							<a href="">
								<img src="img/icons/inst.svg" alt="instagram">
							</a>
						</div>
					</div>
				</div>
				<div class="col-lg-9 main-header">
					<div class="main-header__content">
						<div class="main-header__head d-none d-lg-flex">
							<div class="main-header__block1 d-flex">
								<div class="menuFive mr-3">
				                     <span></span>
				                     <span></span>
				                     <span></span>
				                </div>
				                <p>МЕНЮ</p>
				                <!-- <div class="menu">
				                	<ul>
				                		<li><a href="">Главная</a></li>
				                		<li><a href="">Главная</a></li>
				                		<li><a href="">Главная</a></li>
				                		<li><a href="">Оченьдлиннаястраница очень</a></li>
				                		<li><a href="">Главная</a></li>
				                	</ul>
				                	<ul>
				                		<li><a href="">Главная</a></li>
				                		<li><a href="">Главная</a></li>
				                		<li><a href="">Оченьдлиннаястраница очень</a></li>
				                		<li><a href="">Главная</a></li>
				                		<li><a href="">Оченьдлиннаястраница очень</a></li>
				                	</ul>
				                	<ul>
				                		<li><a href="">Главная</a></li>
				                		<li><a href="">Главная</a></li>
				                		<li><a href="">Оченьдлиннаястраница очень</a></li>
				                	</ul>
				                </div> -->
							</div>
							<div class="main-header__block2">
								<div>
									<p>manager@uralteh.ru</p>
								</div>
								<div>
									<p>(3412) 600-999 <br> <span>Ижевск</span></p>
								</div>
								<div>
									<p>8 (800) 100-53-83 <br> <span>Регионы</span></p>
								</div>
								<div>
									<a data-fancybox data-animation-duration="700" data-src="#orderCall" href="javascript:;">
										<button class="redBtn">Заказать звонок</button>
									</a>
								</div>
								<div>
									<form id="search" class="search">
										<input type="search" placeholder="Поиск">
									</form>
								</div>
								<div>
									<a class="placemark" data-fancybox data-animation-duration="700" data-src="#showMap" href="javascript:;">
										<img src="img/icons/placemark.svg" alt="Карта">
									</a>
								</div>
							</div>
						</div>

								<div class="menu">
				                	<ul>
				                		<li><a href="">Главная</a></li>
				                		<li><a href="">Главная</a></li>
				                		<li><a href="">Главная</a></li>
				                		<li><a href="">Оченьдлиннаястраница очень</a></li>
				                		<li><a href="">Главная</a></li>
				                	</ul>
				                	<ul>
				                		<li><a href="">Главная</a></li>
				                		<li><a href="">Главная</a></li>
				                		<li><a href="">Оченьдлиннаястраница очень</a></li>
				                		<li><a href="">Главная</a></li>
				                		<li><a href="">Оченьдлиннаястраница очень</a></li>
				                	</ul>
				                	<ul>
				                		<li><a href="">Главная</a></li>
				                		<li><a href="">Главная</a></li>
				                		<li><a href="">Оченьдлиннаястраница очень</a></li>
				                	</ul>
				                </div>

						<div class="main-header__link d-none d-lg-block">
							<a href="">
								с <br> о <br> б <br> р <br> а <br> т <br> ь <br> <br> к <br> р <br> а <br> н 
							</a>
						</div>
						<div class="main-header__slider">
							<div class="main-header__slide">
								<h3>Вот вам яркий пример современных тенденций - сплочённость команды.</h3>
								<div class="main-header__advantages">
									<div>
										<img src="img/icons/advantage1.svg" alt="Преимущество">
										<p>Быстрое оформление документов</p>
									</div>
									<div>
										<img src="img/icons/advantage2.svg" alt="Преимущество">
										<p>Наличный и безналичный расчёт</p>
									</div>
									<div>
										<img src="img/icons/advantage3.svg" alt="Преимущество">
										<p>На рынке с 1999 года</p>
									</div>
								</div>
								<button class="whiteBtn">Подробнее</button>
							</div>
							<div class="main-header__slide">
								<h3>Вот вам яркий пример современных тенденций - сплочённость команды.</h3>
								<div class="main-header__advantages">
									<div>
										<img src="img/icons/advantage1.svg" alt="Преимущество">
										<p>Быстрое оформление документов</p>
									</div>
									<div>
										<img src="img/icons/advantage2.svg" alt="Преимущество">
										<p>Наличный и безналичный расчёт</p>
									</div>
									<div>
										<img src="img/icons/advantage3.svg" alt="Преимущество">
										<p>На рынке с 1999 года</p>
									</div>
								</div>
								<button class="whiteBtn">Подробнее</button>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</header>

	<div class="main-header__link d-lg-none d-block">
		<a href="">
			собрать кран
		</a>
	</div>